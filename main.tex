...
%	How does it work in my project?
	...
	\begin{itemize}
		\item \textbf{Retrieval Method:} When a user submits a query, the retriever performs a cosine similarity search against the stored embeddings. This means that the user query is embedded to vectors with a embedding model and compared to the stored vector space. The \texttt{topK} parameter determines how many chunks are retrieved ($k=2$). The best top K number was researched in this thesis by trying out different top K configurations, because lower \texttt{k} can lead to more focused context, while a higher \texttt{k} may introduce noise but offer broader coverage. More about this in results chapter \ref{ch:results}. One additional aspect to take to account is that, the higher K number the more tokens does one query cost and the longer response time is.
